...
- **Parité du préflight avec PDF.js** — les flux JavaScript, XMP et XFA réutilisent le décodeur borné pour Flate, LZW, ASCII85, ASCIIHex, RunLength, leurs chaînes et prédicteurs, au lieu de rejeter les filtres standards non-Flate. Pour chaque widget, le budget de fusion compte également les ressources du flux `/AP` sélectionné entre les ressources héritées du champ et celles de l’AcroForm.
