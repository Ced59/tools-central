...
- **Durcissement final de l’inspecteur PDF** — les corps de flux sont exclus de la collecte des objets de longueur indirecte après validation de leur frontière réelle, y compris lorsque l’entier est stocké dans un `/ObjStm` borné ; les noms de pièces jointes hérités de `/Unix`, `/Mac` ou `/DOS` sont conservés ; les tableaux RichMedia `/Configurations` et `/Instances` et les arbres de rendition Screen sont plafonnés à 1 000 000 d’entrées matérialisées par annotation ; les noms et descriptions média partagés disposent d’un plafond agrégé de 32 MiB ; les JavaScript de page ne sont inventoriés qu’une fois.
