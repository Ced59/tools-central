...
- **Déchiffrement PDF borné avant matérialisation** — l’inspecteur charge un Worker PDF.js dédié dont les `DecodeStream` et sorties progressives de `DecompressionStream` refusent toute croissance au-delà de 32 MiB avant allocation agrégée. Chaque allocation supplémentaire et chaque chunk décompressé débitent aussi, avant conservation, un plafond partagé par tous les flux du document dans ce Worker éphémère ; les API XMP, XFA, JavaScript, champs, signatures, plan et annotations sont ensuite appelées séquentiellement et leurs sorties partagent le budget documentaire applicatif. Le Worker générique utilisé par les autres outils PDF n’est pas modifié. La génération versionnée échoue explicitement si l’une des trois ancres du bundle PDF.js change. `/Filter null` est traité comme un flux sans filtre dans le préflight des Object Streams comme dans les lecteurs XMP, XFA et JavaScript.
