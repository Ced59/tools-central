...
- **Tolérance syntaxique PDF** — les entiers non négatifs peuvent porter un signe `+` explicite ; le scanner saute la valeur complète de chaque clé d’extension, y compris un nom, une référence, un tableau ou un dictionnaire contenant des noms homonymes de clés critiques. Les extensions `DecodeParms` peuvent également contenir des noms ou tableaux homonymes des paramètres standards sans être interprétées comme des clés directes. Un `/Type` de flux indirect est résolu par l’offset xref autoritaire vers un nom ou `null` ; une référence ambiguë reste rejetée avant PDF.js. Les scalaires de prédicteur indirects sont eux aussi résolus par cette autorité, qu’ils soient stockés directement ou dans l’Object Stream actif désigné par une entrée xref de type 2 ; l’amorçage d’un flux xref couvre les références directes nécessaires à la construction de cette autorité. Un PDF valide sans page conserve enfin l’inspection de tous ses signaux documentaires sans appel à `getPage`.
