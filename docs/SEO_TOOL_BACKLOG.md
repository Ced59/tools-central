...
- **PDF vers images** — implémenté en Clean Architecture, rendu PDF.js entièrement local dans un Web Worker avec `OffscreenCanvas`, sélection bornée de pages, formats PNG/JPEG/WebP, DPI et qualité réglables, estimation préventive du budget pixels, aperçu et ZIP chargé à la demande, tests unitaires et E2E mobile. Route publiée en français : `/categories/dev/pdf/pdf-to-images` ; les autres locales restent hors index jusqu’à leur relecture.
