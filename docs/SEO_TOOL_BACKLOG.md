...
- **Déchiffrement PDF borné avant matérialisation** — l’inspecteur charge un Worker PDF.js dédié dont les `DecodeStream` et sorties progressives de `DecompressionStream` refusent toute croissance au-delà de 32 MiB avant allocation agrégée. Chaque allocation supplémentaire et chaque chunk décompressé débitent aussi, avant conservation, un plafond partagé par tous les flux du document dans ce Worker éphémère ; le fallback Brotli synchrone est refusé avant son décodeur non progressif, tandis que le chemin `DecompressionStream` reste borné. Comme l’inspecteur ne rend aucune page, les chemins directs JPEG, `ImageDecoder`, CCITT, JBIG2 et JPX sont refusés avant leur première allocation dans ce seul Worker. Les API XMP, XFA, JavaScript, champs, signatures, plan et annotations sont ensuite appelées séquentiellement et leurs sorties partagent le budget documentaire applicatif. Le Worker générique utilisé par les autres outils PDF n’est pas modifié. La génération versionnée échoue explicitement si l’une des neuf ancres du bundle PDF.js change. `/Filter null` est traité comme un flux sans filtre dans le préflight des Object Streams comme dans les lecteurs XMP, XFA et JavaScript ; une référence indirecte vers `null` dans un tableau `DecodeParms` est équivalente à un `null` direct. Une référence `/Encrypt` dont l’entrée xref autoritaire pointe vers un `null` compressé est résolue dans son Object Stream borné, tandis que toute résolution incertaine reste fermée. Les alias de l’arbre de noms renvoyés plusieurs fois par PDF.js pour un même FileSpec déchiffré sont regroupés après rapprochement avec l’identité structurelle.
- **Tolérance syntaxique PDF** — les entiers peuvent porter un signe explicite ; le scanner saute la valeur complète de chaque clé d’extension, y compris un nom, une référence, un tableau ou un dictionnaire contenant des noms homonymes de clés critiques. Les extensions `DecodeParms` peuvent également contenir des noms ou tableaux homonymes des paramètres standards sans être interprétées comme des clés directes. Les paramètres signés ne sont validés que lorsqu’ils sont utilisés par le filtre actif. Un `/Type` de flux indirect est résolu par l’autorité xref vers un nom ou `null`, y compris lorsque la valeur se trouve dans l’Object Stream actif désigné par une entrée type 2 ; une référence ambiguë reste rejetée avant PDF.js. Les scalaires de prédicteur indirects suivent la même autorité, qu’ils soient stockés directement ou sous forme compressée ; l’amorçage d’un flux xref couvre les références directes nécessaires à la construction de cette autorité. Un PDF valide sans page conserve enfin l’inspection de tous ses signaux documentaires sans appel à `getPage`.
