...
- **Inspecteur de confidentialité PDF** — implémenté en Clean Architecture avec analyse PDF.js dans un Worker isolé, prise en charge des PDF protégés par mot de passe, inventaire des métadonnées/XMP, scripts sans exposition du code, actions automatiques, pièces jointes, URL, formulaires sans exposition des valeurs, signatures et chiffrement. Les ressources restent locales, les liens ne sont jamais suivis et le rapport JSON explicite qu’il ne vaut ni verdict antivirus, ni analyse visuelle, ni validation cryptographique. Route publiée en français : `/categories/dev/pdf/pdf-privacy-inspector` ; les autres locales restent hors index jusqu’à leur relecture.
