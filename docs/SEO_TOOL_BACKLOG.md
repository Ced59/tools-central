...
- **Durcissement final de l’inspecteur PDF** — les corps de flux sont exclus de la collecte des objets de longueur indirecte après validation de leur frontière réelle, les noms de pièces jointes hérités de `/Unix`, `/Mac` ou `/DOS` sont conservés, et les tableaux RichMedia `/Configurations` et `/Instances` partagés sont plafonnés à 1 000 000 d’entrées matérialisées par annotation avant PDF.js.
