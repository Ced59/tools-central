...
- **Garantie du budget de filtres PDF** — chaque sortie intermédiaire d’une chaîne et chaque reconstruction de prédicteur consomme le plafond agrégé, y compris si la sortie finale est plus petite. Les filtres et dictionnaires `DecodeParms` indirects sont résolus par la xref autoritative ; lors de l’amorçage d’un flux xref, une déclaration directe unique n’est acceptée que si les entrées décodées confirment son offset. Le prédicteur TIFF accepte les profondeurs standard 1, 2, 4, 8 et 16 bits, et les signatures nommées après déchiffrement restent dédupliquées par occurrence.
