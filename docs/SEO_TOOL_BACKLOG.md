...
- **Garantie du budget de filtres PDF** — chaque sortie intermédiaire d’une chaîne et chaque reconstruction de prédicteur consomme le plafond agrégé, y compris si la sortie finale est plus petite. Les dictionnaires `DecodeParms` indirects sont résolus par la xref autoritative et le prédicteur TIFF accepte les profondeurs standard 1, 2, 4, 8 et 16 bits.
