...
- Après classification du mot de passe, un Worker PDF.js dédié à l’inspecteur refuse désormais toute croissance d’un `DecodeStream` ou sortie progressive de `DecompressionStream` au-delà de 32 MiB avant allocation agrégée. Les API susceptibles de matérialiser XMP, XFA ou JavaScript sont appelées séquentiellement puis débitées du budget documentaire agrégé ; le Worker PDF.js standard des autres outils reste inchangé. La préparation vérifie deux ancres uniques du bundle épinglé et fait échouer le build si une future mise à jour PDF.js invalide le garde-fou. Le préflight accepte aussi `/Filter null` comme l’absence de filtre, conformément à la sémantique PDF.
- Le parseur brut accepte le signe positif explicite des entiers PDF non négatifs et ignore les clés homonymes présentes dans les valeurs d’extension, notamment les tableaux ou noms de `DecodeParms`, sans relâcher la validation des paramètres directs.
