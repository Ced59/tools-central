...
| Bundle initial de production | 656,90 kB lors du premier build corrigé | 658,81 kB brut, 166,21 kB transféré estimé ; PDF.js et les moteurs documentaires restent dans des chunks lazy (Worker d’inspection PDF : 1,02 MB brut, 288,62 kB transféré) |
| Vulnérabilités npm | 58, dont 5 critiques | 0 |
| Tests unitaires | suite non compilable | 139 fichiers, 518 tests verts sous Vitest |
| Couverture | aucun seuil | 51,76 % statements, 40,24 % branches, 50,84 % fonctions, 55,35 % lignes |
...
Dernière mesure Playwright locale en configuration CI sur l’accueil mobile : LCP 2 244 ms, CLS 0,0043, `DOMContentLoaded` 845,6 ms et interaction thème 96 ms. Ce sont des garde-fous de laboratoire, variables selon la machine, pas des Core Web Vitals terrain.
...
- Les Workers PDF.js restent paresseux : celui de l’inspecteur pèse 1,02 MB brut (288,62 kB transféré estimé) et ne charge son moteur de parsing qu’à l’usage ; mémoire et temps doivent rester surveillés sur de gros documents.
