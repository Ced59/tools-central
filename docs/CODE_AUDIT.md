...
| Bundle initial de production | 656,90 kB lors du premier build corrigé | 658,81 kB brut, 166,20 kB transféré estimé ; PDF.js et les moteurs documentaires restent dans des chunks lazy (Worker d’inspection PDF : 1,02 MB brut, 288,82 kB transféré) |
| Vulnérabilités npm | 58, dont 5 critiques | 0 |
| Tests unitaires | suite non compilable | 139 fichiers, 523 tests verts sous Vitest |
| Couverture | aucun seuil | 51,98 % statements, 40,52 % branches, 50,89 % fonctions, 55,58 % lignes |
...
Dernière mesure Playwright locale en configuration CI sur l’accueil mobile : LCP 2 288 ms, CLS 0,0043, `DOMContentLoaded` 874,3 ms et interaction thème 94 ms. Ce sont des garde-fous de laboratoire, variables selon la machine, pas des Core Web Vitals terrain.
...
- Les Workers PDF.js restent paresseux : celui de l’inspecteur pèse 1,02 MB brut (288,82 kB transféré estimé) et ne charge son moteur de parsing qu’à l’usage ; mémoire et temps doivent rester surveillés sur de gros documents.
