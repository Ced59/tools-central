...
Le dépôt reste un produit en migration, pas une Clean Architecture achevée. Quatorze features servent désormais de références (`percentage-of-number`, le moteur de nettoyage PDF, `serp-snippet-preview`, `robots-txt-builder`, `sitemap-xml-builder`, `hreflang-checker`, `structured-data-extractor`, `html-head-auditor`, `software-application-schema-builder`, `pdf-to-images`, `images-to-pdf`, `ooxml-metadata-cleaner`, `pdf-privacy-inspector` et `csv-json-converter`), mais l’essentiel du code historique reste organisé par composants. Les prochaines PR doivent donc réduire la dette par tranche fonctionnelle, sans refonte globale. La priorité produit/SEO n’est pas de publier les 106 outils encore indisponibles : elle est d’améliorer les 71 outils réels, faire relire les traductions et livrer les nouvelles intentions une par une.
...
| Bundle initial de production | 656,90 kB lors du premier build corrigé | 659,80 kB brut, 166,33 kB transféré estimé ; PDF.js et les moteurs documentaires restent dans des chunks lazy (Worker d’inspection PDF : 1,08 MB brut, 303,80 kB transféré) |
| Vulnérabilités npm | 58, dont 5 critiques | 0 |
| Tests unitaires | suite non compilable | 144 fichiers, 606 tests verts sous Vitest, plus 5 tests Node du Worker PDF.js généré |
| Couverture | aucun seuil | 57,24 % statements, 48,24 % branches, 54,04 % fonctions, 61,23 % lignes |
| E2E | aucun | 16 parcours Playwright verts, dont des scénarios PDF de confidentialité et CSV ↔ JSON réels |
...
Dernière mesure Playwright locale sur l’accueil mobile : LCP 2 008 ms, CLS 0,0043, `DOMContentLoaded` 312,6 ms et interaction thème 68,0 ms. Ce sont des garde-fous de laboratoire, variables selon la machine, pas des Core Web Vitals terrain.
...
- Après classification du mot de passe, un Worker PDF.js dédié à l’inspecteur refuse désormais toute croissance d’un `DecodeStream` ou sortie progressive de `DecompressionStream` au-delà de 32 MiB avant allocation agrégée. Chaque croissance et chaque chunk progressif débitent en plus un budget partagé par tous les flux du document dans ce Worker éphémère, avant leur conservation en mémoire. Le fallback Brotli synchrone, qui matérialise sa sortie entière sans capacité de contrôle préalable, est refusé avant son décodeur natif ; le chemin `DecompressionStream` reste autorisé sous le plafond progressif. L’inspecteur ne rendant aucune page, son Worker dédié refuse aussi JPEG, `ImageDecoder`, CCITT, JBIG2 et JPX avant leur première allocation ; les PDF ordinaires qui contiennent des images restent inspectables tant qu’aucun flux sémantique n’emploie abusivement un filtre d’image. Les API susceptibles de matérialiser XMP, XFA ou JavaScript sont appelées séquentiellement puis débitées du budget documentaire applicatif ; le Worker PDF.js standard des autres outils reste inchangé. La préparation vérifie neuf ancres uniques du bundle épinglé et fait échouer le build si une future mise à jour PDF.js invalide le garde-fou ; cinq tests Node couvrent le plafond inter-flux, le débit avant rétention, le refus des décodeurs directs, le refus du fallback Brotli et l’échec fermé si les ancres changent. Le préflight et le lecteur borné des flux texte XMP, XFA et JavaScript acceptent aussi `/Filter null` comme l’absence de filtre, conformément à la sémantique PDF ; une référence indirecte vers `null` dans un tableau `DecodeParms` est traitée comme son équivalent direct.
...
- Une entrée `/Encrypt` indirecte est reconnue comme absente lorsque la xref active désigne autoritairement un objet `null`, y compris une entrée type 2 stockée dans un `/ObjStm` borné dont `/N`, `/First`, `/Filter` ou `/DecodeParms` dépend de valeurs compressées dans une chaîne ciblée d’autres Object Streams. Cet amorçage est mis en cache, protégé contre les cycles et débité du budget partagé ; aucun autre flux n’est exploré. Un objet introuvable, ambigu, remplacé ou non nul reste traité prudemment comme un chiffrement réel.
- Le parseur brut accepte le signe explicite des entiers PDF et saute atomiquement la valeur complète des clés d’extension — nom, référence, tableau ou dictionnaire — afin de ne jamais relire un nom homonyme comme une clé critique de flux. Il ignore notamment les noms de paramètres présents dans les extensions `DecodeParms`, sans relâcher la validation des paramètres réellement utilisés par le filtre actif : les paramètres de prédicteur ne sont contrôlés que pour Flate/LZW, leurs dimensions seulement lorsqu’un prédicteur est activé, et `EarlyChange` seulement pour LZW.
- La valeur `/Type` d’un flux peut être un nom direct, `null` ou une référence indirecte. Une référence n’est acceptée qu’après résolution autoritative du nom ou de `null` depuis la xref active, soit à l’offset classique, soit dans l’Object Stream actif désigné par une entrée type 2 ; une valeur absente reste ordinaire et une référence ambiguë échoue fermement avant PDF.js. Les scalaires `Predictor`, `Colors`, `BitsPerComponent`/`BPC`, `Columns` et `EarlyChange` de `DecodeParms` suivent la même résolution autoritative lorsqu’ils sont indirects, qu’ils soient des objets classiques ou compressés, y compris pendant l’amorçage borné d’un flux xref lorsque cette référence peut être directe.
