...
- Livraison de `json-diff` dans une tranche verticale complète, avec parseur JSON strict refusant clés dupliquées, nombres non fidèles et Unicode mal formé, diff par chemin/type/valeur, association des tableaux d’objets par clé JSON Pointer, détection des déplacements, chemins ignorés, patch RFC 6902 applicable, rapport borné avant sérialisation, import UTF-8, Worker annulable, partage sans données, interface mobile accessible et 50 tests ciblés.
