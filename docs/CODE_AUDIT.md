...
| Bundle initial de production | 656,90 kB lors du premier build corrigé | 658,81 kB brut, 166,19 kB transféré estimé ; PDF.js et les moteurs documentaires restent dans des chunks lazy (Worker d’inspection PDF : 1,08 MB brut, 303,05 kB transféré) |
...
| Couverture | aucun seuil | 56,62 % statements, 47,39 % branches, 53,24 % fonctions, 60,63 % lignes |
...
Dernière mesure Playwright locale en configuration CI sur l’accueil mobile : LCP 2 160 ms, CLS 0,0043, `DOMContentLoaded` 434,6 ms et interaction thème 63,5 ms. Ce sont des garde-fous de laboratoire, variables selon la machine, pas des Core Web Vitals terrain.
...
- Une entrée `/Encrypt` indirecte est reconnue comme absente lorsque la xref active désigne autoritairement un objet `null`, y compris une entrée type 2 stockée dans un `/ObjStm` borné dont `/N` ou `/First` dépend de scalaires compressés dans une chaîne ciblée d’autres Object Streams. Cet amorçage est mis en cache, protégé contre les cycles et débité du budget partagé ; aucun autre flux n’est exploré. Un objet introuvable, ambigu, remplacé ou non nul reste traité prudemment comme un chiffrement réel.
