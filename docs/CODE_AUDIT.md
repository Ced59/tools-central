...
| Bundle initial de production | 656,90 kB lors du premier build corrigé | 658,81 kB brut, 166,14 kB transféré estimé ; PDF.js et les moteurs documentaires restent dans des chunks lazy (Worker d’inspection PDF : 1,01 MB brut, 288,08 kB transféré) |
| Vulnérabilités npm | 58, dont 5 critiques | 0 |
| Tests unitaires | suite non compilable | 139 fichiers, 514 tests verts sous Vitest |
| Couverture | aucun seuil | 51,53 % statements, 39,79 % branches, 50,71 % fonctions, 55,09 % lignes |
...
| Traductions secondaires | marqueurs incomplets non bloqués | 184 498 segments, 0 `TODO`, 0 warning technique |
| Dette éditoriale source | 96 `TODO` | 0 `TODO` |
| Inventaire SEO | absent | 297 opportunités + shortlist prioritaire de 30 |

Dernière mesure Playwright locale en configuration CI sur l’accueil mobile : LCP 1 880 ms, CLS 0, `DOMContentLoaded` 449,5 ms et interaction thème 127,3 ms. Ce sont des garde-fous de laboratoire, variables selon la machine, pas des Core Web Vitals terrain.
...
- Les Workers PDF.js restent paresseux : celui de l’inspecteur pèse 1,01 MB brut (287,61 kB transféré estimé) et ne charge son moteur de parsing qu’à l’usage ; mémoire et temps doivent rester surveillés sur de gros documents.
