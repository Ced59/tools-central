...
| Tests unitaires | suite non compilable | 158 fichiers, 840 tests verts sous Vitest, plus 5 tests Node du Worker PDF.js généré |
...
- Livraison de `csv-json-converter` dans une tranche verticale complète, avec parseur CSV compatible champs cités et multilignes, détection du séparateur, inférence de types sans arrondir les décimales, rejet des nombres JSON qui perdraient leur représentation, mapping/sélection/réordonnancement des colonnes, désambiguïsation globale des en-têtes, aplatissement JSON sans collision silencieuse, protection des cellules et en-têtes contre les formules de tableur, diagnostics bornés et annoncés aux technologies d’assistance, limites mémoire, Worker annulable, import UTF-8 strict, copie et export locaux, accessibilité mobile et 61 tests ciblés.
- Livraison de `json-diff` dans une tranche verticale complète, avec parseur JSON strict refusant clés dupliquées, nombres non fidèles et Unicode mal formé, diff par chemin/type/valeur, association des tableaux d’objets par clé JSON Pointer, détection des déplacements, chemins ignorés, rejet explicite des exclusions incompatibles avec un patch sans perte — y compris lors d’un déplacement par clé —, patch RFC 6902 applicable, rapport borné avant sérialisation, import UTF-8, Worker annulable, partage sans données, interface mobile accessible et 52 tests ciblés.
- Livraison de `json-schema-validator` dans une tranche verticale complète, avec parseur JSON strict, Draft 7, 2019-09 et 2020-12, formats Ajv activables, références internes sans chargement réseau, diagnostic par chemins d’instance et de schéma, proposition corrigée limitée aux transformations prudentes puis revalidée, plafonds sur profondeur, nœuds, opérations estimées, erreurs, motifs et sorties, import UTF-8, Worker annulable, partage sans données et interface mobile accessible.
